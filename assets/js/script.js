...
const cityFormHandler = function (cityName) {
  fetch(urlOne + cityName + "&appid=" + weatherKey + "&units=imperial").then(async function (response) {
    if (response.ok) {
      ...
      fetch(urlTwo + lat + "&lon=" + lon + "&appid=" + weatherKey + "&units=imperial").then(async function (response2) {
        if (response2.ok) {
          return response2.json().then(function (data2) {
            dailyArray = data2.daily;
...
function crtSearchDiv() {
  const buildSearchDiv = document.createElement("div");
  buildSearchDiv.classList.add("searchDiv", "col-3");
  buildSearchDiv.id = "searchDiv";
  return buildSearchDiv;
}

function crtSearchForm() {
  const buildSearchForm = document.createElement("form");
  buildSearchForm.classList.add("cityForm");
  buildSearchForm.id = "cityForm";
  return buildSearchForm;
}

function crtSearchInput() {
  const buildSearchInput = document.createElement("input");
  buildSearchInput.type = "text";
  buildSearchInput.name = "citySearch";
  buildSearchInput.classList.add("cityName");
  buildSearchInput.id = "cityName";
  buildSearchInput.placeholder = "City Name";
  return buildSearchInput;
}

function crtSearchBtn() {
  const buildSearchBtn = document.createElement("button");
  buildSearchBtn.classList.add("submit");
  buildSearchBtn.type = "submit";
  buildSearchBtn.textContent = "Search";
  return buildSearchBtn;
}
...
function uviColor(tUvi) {
  if (tUvi <= 2.99 || tUvi <= "2.99") {
    return "uvi12";
  } else if ((tUvi >= 3.0 && tUvi <= 5.99) || (tUvi >= "3.00" && tUvi <= "5.99")) {
    return "uvi345";
  } else if ((tUvi >= 6.0 && tUvi <= 7.99) || (tUvi >= "6.00" && tUvi <= "7.99")) {
    return "uvi67";
  } else if ((tUvi >= 8.0 && tUvi <= 10.99) || (tUvi >= "8.00" && tUvi <= "10.99")) {
    return "uvi8910";
  } else if (tUvi >= 11.0 || tUvi >= "11.00") {
    return "uvi11";
  } else return "uviNone";
}
...
function crtIcon(tIcon) {
  for (let x = 0; x < tIcon.length; x++) {
    const getIcon = tIcon[x].icon;
    const buildIcon = document.createElement("span");
    buildIcon.classList.add("tIcon");
    buildIcon.id = "tIcon-" + x;
    buildIcon.innerHTML = '<img src = "http://openweathermap.org/img/wn/' + getIcon + '@2x.png" class="cIcon" alt="current weather icon">';
    return buildIcon;
  }
}
...
function crtForecastWrapper() {
  ...
  forecastWrapper.classList.add("forecastWrapper");
...
$(document).ready(function () {
  ...
  const buildSearchDiv = crtSearchDiv();
  const buildSearchForm = crtSearchForm();
  const buildSearchInput = crtSearchInput();
  const buildSearchBtn = crtSearchBtn();

  document.querySelector(".container").appendChild(buildBanner);
  document.querySelector(".container").appendChild(buildRow);
  document.querySelector("#row").appendChild(buildSearchDiv);
  document.querySelector("#searchDiv").appendChild(buildSearchForm);
  document.querySelector("#cityForm").appendChild(buildSearchInput);
  document.querySelector("#cityForm").appendChild(buildSearchBtn);
